 ...
 */
void
bson_dec128_to_string (bson_dec128_t dec,        /* IN  */
                       char         *str)        /* OUT */
{
   uint32_t COMBINATION_MASK = 0x1f,   /* Extract least significant 5 bits */
            EXPONENT_MASK = 0x3fff,    /* Extract least significant 14 bits */
            COMBINATION_INFINITY = 30, /* Value of combination field for Inf */
            COMBINATION_NAN = 31,      /* Value of combination field for NaN */
            EXPONENT_BIAS = 6176;      /* decimal128 exponent bias */

   char *str_out = str,                 /* output pointer in string */
        significand_str[35]; /* decoded significand digits */
   ...
   uint32_t high,                    /* bits 0 - 31 */
            ...
            midl,                    /* bits 64 - 95 */
            low,                     /* bits 96 - 127 */
            combination,             /* bits 1 - 5 */
            biased_exponent,         /* decoded biased exponent (14 bits) */
            significand_digits = 0,  /* the number of significand digits */
            significand[36] = { 0 }; /* the base-10 digits in the significand */
   ...
   uint32_t *significand_read = significand;
   if (is_zero) {
   ...
   int32_t scientific_exponent = significand_digits - 1 + exponent;
   ...
   if (scientific_exponent >= 12 || scientific_exponent <= -4 ||
      ...
      *(str_out++) = *(significand_read++) + '0';
      ...
      /* Exponent */
      *(str_out++) = 'E';
      sprintf (str_out, "%+d", scientific_exponent);
   } else {
 ...
 *-------------------------------------------------------------------------
 */
static _bson_uint128_6464_t
_mul_64x64 (uint64_t left,
           uint64_t right)
{
                         ...
                         bson_dec128_t *dec) /* OUT */
{
   ...
   size_t significant_digits = 0,    /* Total number of significant digits
          ...
          ndigits_read = 0,   /* Total number of significand digits read */
          ndigits = 0,        /* Total number of digits (no leading zeros) */
          radix_position = 0, /* The number of the digits after radix */
          first_nonzero = 0;  /* The index of the first non-zero in *str* */

   uint16_t digits[BSON_DEC128_MAX_DIGITS] = { 0 },
            ndigits_stored = 0,      /* The number of digits in digits */
           *digits_insert = digits;  /* Insertion pointer for digits */
   size_t first_digit = 0,      /* The index of the first non-zero digit */
          last_digit = 0;       /* The index of the last digit */
   ...
   if (isalpha (*str_read) || *str_read == '.' || !isdigit(*str_read)) {
   ...
   if (ndigits_stored == 0) {  /* value is zero */
      first_digit = 0;
      last_digit = 0;
      digits[0] = 0;
      ndigits = 1;
   ...
   significand = _mul_64x64 (significand_high,
   ...
   uint16_t biased_exponent = (exponent + (int16_t)BSON_DEC128_EXPONENT_BIAS);
