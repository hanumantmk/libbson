 ...
 *       The result is valid while @iter is valid.
 *
 * Returns:
 *    A bson_dec128_t that should not be modified or freed.
 *
 * Side Effects:
 *    None.
 *
...
bson_iter_dec128 (const bson_iter_t *iter) /* IN */
{
   ...
   fail.low = 0;
   fail.high = 0;
   bson_return_val_if_fail (iter, fail);

   if (ITER_TYPE (iter) == BSON_TYPE_DEC128) {
      return bson_iter_dec128_unsafe (iter);
   }
                     ...
                     void                 *data)    /* IN */
{
   ...
   while (bson_iter_next (iter)) {
      ...
      case BSON_TYPE_DEC128:
         ...
         if (VISIT_DEC128 (iter, key, bson_iter_dec128 (iter), data)) {
            return true;
         }
...
bson_iter_value (bson_iter_t *iter) /* IN */
{
   ...
   case BSON_TYPE_INT64:
      ...
      break;
   case BSON_TYPE_DEC128:
      value->value.v_dec128 = bson_iter_dec128 (iter);
      break;
   case BSON_TYPE_NULL:
   case BSON_TYPE_UNDEFINED:
   case BSON_TYPE_MAXKEY:
